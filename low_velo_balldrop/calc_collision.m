% INPUT
% -------------------------------------------------------------------------
% fdat = simulation data (use si units)
...
% LW = 1.5;
% FS = 25;
% makePlot(time, tempA, 'time(sec)', 'acceleration (m/s^2)', '', LW, FS);
% hold on
% makePlot(time, acc, 'time(sec)', 'acceleration (m/s^2)', '', LW+1, FS);
% legend('Simulation data', 'after applying a low-pass filter');
% grid on
% xlim([min(time), max(time)]);

% Define tstart as the step right before the end of freefall
% index of start of collision
...
fprintf("penetration depth: [%e, %e, %e] m, max acc: %e m/s^2, collision time: [%e, %e, %e] sec\n", ...
         pdat.zstop1 - pdat.zinit, pdat.zstop2 - pdat.zinit, pdat.zstop3 - pdat.zinit,  ...
         pdat.apeak, pdat.tstop1 - pdat.tstart, pdat.tstop2 - pdat.tstart, pdat.tstop3 - pdat.tstart);