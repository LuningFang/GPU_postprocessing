function convertDATtoVTK(input, output)


[verticePosArray, facetsArray, numVertices, numFacets] = extractMeshInfo(input);

writeVTK(verticePosArray, facetsArray, numVertices, numFacets, output);
end

function [verticePosArray, facetsArray, numVertices, numFacets] = extractMeshInfo(filename)
% the line number when num vertices is printed
extraNumLines = 7;
fid = fopen(filename);
